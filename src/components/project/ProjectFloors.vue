<template>
    <div class="project-floors">
        <div class="project-floor" v-for="(floor, key) in floors">
            <div class="floor-upload-button" v-if="floor.image == ''">
                <div class="logo-upload" @click="uploadImage(key)">Upload floor image
                </div>
                <input type="file" :floor-number="key" @change="uploadFloorImage" :id="'floor-image-' + key"
                       style="display: none">
            </div>
            <div class="floor-image" v-if="floor.image != ''"></div>
